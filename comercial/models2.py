...
class CompraNacional(models.Model):
    ...
    precio_compra_exportacion = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="$ Compra Exportacion", validators=[MinValueValidator(0.0)])
    precio_compra_nacional = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="$ Compra Nacional", blank=True, null=True, validators=[MinValueValidator(0.0)])
...
class VentaNacional(models.Model):
    ...
    cantidad_empaque_vendida = models.PositiveIntegerField(verbose_name="Cantidad Empaque Vendida", validators=[MinValueValidator(0)], blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        if not self.cantidad_empaque_vendida:  # Si no se ha modificado, ingreso la cantidad de empaque de compra.
            self.cantidad_empaque_vendida = self.compra_nacional.cantidad_empaque
